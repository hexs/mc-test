...
class FX3U:
    ...
    def __init__(
            ...
            port: int,
            debug: bool = False,
            min_interval: float = 0.05,  # ขั้นต่ำระหว่างคำสั่ง (วินาที)
            max_retries: int = 3,  # จำนวนครั้งที่ retry ถ้า connect refused
    ) -> None:
        self.ip = ip
        self.port = port
        self.debug = debug
        ...
        self._last_tx = 0.0  # time.monotonic() ของคำสั่งล่าสุด
    ...
    def _exchange(self, cmd_hex: str) -> str:
        ...
        for attempt in range(self.max_retries):
            # ----- rate limit: หน่วงให้ห่างอย่างน้อย min_interval -----
            now = time.monotonic()
            delta = now - self._last_tx
            if delta < self.min_interval:
                time.sleep(self.min_interval - delta)

            # อัพเดทเวลา หลังจากหน่วงเรียบร้อย
            ...
            try:
                ...
                if not data:
                    raise RuntimeError("Empty response from PLC")
                ...
                last_err = e
                # ถ้าเป็น connection refused และยังมีโอกาส retry → หน่วงแล้วลองใหม่
                # errno 111 บน Linux = ECONNREFUSED
                if getattr(e, "errno", None) == 111 and attempt < self.max_retries - 1:
                    if self.debug:
                        print(f"Connection refused, retry {attempt + 1}/{self.max_retries - 1} ...")
                    time.sleep(self.min_interval)
                    continue
                # อย่างอื่น / หมดโควต้า retry → ยอมแพ้
                raise

        # เผื่อหลุดจาก loop โดยไม่ได้ raise ข้างใน (ปกติไม่ควรเกิด)
        raise RuntimeError(f"Exchange failed after retries: {last_err}")
