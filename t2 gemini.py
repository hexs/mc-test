...
def _exchange(cmd_hex: str) -> str:
    ...
    try:
        with socket.create_connection((PLC_IP, PLC_PORT), timeout=2.0) as sock:
            # เพิ่ม socket option เล็กน้อยเผื่อช่วยเรื่องการ reuse (อาจจะไม่จำเป็นแต่ใส่ไว้ดีกว่า)
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            sock.sendall(cmd_hex.encode("ascii"))
            data = sock.recv(4096)
    except ConnectionRefusedError:
        print(f"!!! Connection Refused on {PLC_IP}:{PLC_PORT} !!!")
        print("PLC might be busy cleaning up previous connection.")
        raise

    # --- จุดสำคัญ: เพิ่ม Delay หลังจากปิด Socket ---
    # FX3U-ENET-L ต้องการเวลาในการเคลียร์ connection เก่า
    # ลองเริ่มที่ 0.1 วินาที (100ms) ถ้ายังมีปัญหาให้เพิ่มเป็น 0.2 หรือ 0.5
    time.sleep(0.1)
    # -------------------------------------------
