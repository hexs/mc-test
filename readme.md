...
ถ้าใน if __name__ == "__main__": ไม่มี time.sleep(0.1) จะเจอปัญหา Connection refused บ่อยๆ เพราะ PLC รับการเชื่อมต่อใหม่ถี่เกินไป
Key Changes Made:
1. __init__: Now calls self._connect() to create self.sock. It stores the socket object instead of throwing it away.
2. _exchange:
   - Uses self.sock.sendall (the existing socket) instead of socket.create_connection.
   - Adds a try...except block. If the connection drops (Broken Pipe), it calls _connect() automatically to reconnect and tries the command again.
3. close: Added a method to cleanly close the socket when the program is done.
With this approach, you can run the loop as fast as Python and the PLC can communicate, without artificial delays.
